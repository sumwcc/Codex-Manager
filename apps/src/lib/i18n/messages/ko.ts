...
export const KO_MESSAGES: MessageCatalog = {
  ...
  "预热任务正在执行，关闭弹窗不会取消后端任务。":
    "예열 작업이 실행 중입니다. 이 창을 닫아도 백엔드 작업은 취소되지 않습니다.",
  "预热任务已完成。": "예열 작업이 완료되었습니다.",
  总数: "총계",
  跳过: "건너뜀",
  等待中: "대기 중",
  进行中: "진행 중",
  ...
  结果消息: "결과 메시지",
  暂无数据: "데이터 없음",
  "账号不可用或缺少有效 token": "계정을 사용할 수 없거나 유효한 token이 없습니다",
  "账号 AT/RT 已刷新：RT {returned}，轮换 {changed}":
    "계정 AT/RT 새로고침 완료: RT {returned}, 회전 {changed}",
  已返回: "반환됨",
  未返回: "반환 안 됨",
  已轮换: "회전됨",
  未轮换: "회전 안 됨",
  ...
  "RT 已返回{returned}个，RT 已轮换{changed}个，RT 未返回{missing}个":
    "RT 반환 {returned}개, RT 회전 {changed}개, RT 미반환 {missing}개",
  "本次可能只刷新了 AT，服务端未返回新 RT":
    "이번에는 AT만 새로고침되었을 수 있으며 서버가 새 RT를 반환하지 않았습니다",
  ...
  "RT 已过期": "RT 만료됨",
  "RT 无效": "RT 유효하지 않음",
  "RT 刷新被拒绝": "RT 새로고침 거부됨",
  用量接口未授权: "사용량 API 인증되지 않음",
  用量接口无权限: "사용량 API 권한 없음",
  用量已耗尽: "사용량 소진",
  账号已停用: "계정 비활성화됨",
  工作区已停用: "워크스페이스 비활성화됨",
  "Token 刷新成功": "Token 새로고침 성공",
  续费时间: "갱신 시간",
  已订阅: "구독 중",
  未订阅: "미구독",
  取消优先: "우선 해제",
  ...
  "顺序优先：按账号候选顺序优先尝试；均衡轮询：按“平台密钥 + 模型”维度轮询可用账号；轮训-用量刷新时间：优先使用 7 天额度窗口重置时间更远的账号，同等情况下 7 天已用比例更低的账号靠前，最后再按用量刷新时间最旧优先。":
    "순서 우선: 계정 후보 순서대로 시도합니다. 균형 라운드로빈: “API 키 + 모델” 기준으로 사용 가능한 계정을 순환합니다. 순환 - 사용량 새로고침 시간: 7일 할당량 창 재설정 시간이 더 먼 계정을 우선하고, 같으면 7일 사용률이 낮은 계정, 그다음 사용량 새로고침 시간이 가장 오래된 계정을 우선합니다.",
